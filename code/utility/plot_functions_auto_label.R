...
gilbert_cp_path = file.path(metadata_dir, 'gilbert21_mod_correlate_protection.xlsx')
feng_cp_path = file.path(metadata_dir, 'feng21_chAd_correlate_protection.xlsx')
ag_colors_path = file.path(metadata_dir, 'ag_colors.csv')
# get correlates of protection


gilbert_cp <- readxl::read_excel(gilbert_cp_path)
feng_cp <- readxl::read_excel(feng_cp_path)
...
mapColors <- read.csv(file = ag_colors_path, row.names = 'Antigen', header = TRUE)

colors <- c(mapColors[c("B.1.617.2","B.1.1.7","B.1.351", "WA1", "WA1", "WA1", "WA1", "WA1"),"Color"], "grey", mapColors[c("BA.1","BA.1","BA.2", "BA.1.1", "BA.2.12.1", "BA.3", "BA.4", "BA.5", "BA.5", "BA.2.75"),"Color"], "red","#BB0D3A", "#F2ACB9" )
names(colors) <- c("Delta", "Alpha", "Beta", "Wu-1", "D614G","WT", "B.1","WA1", "Wu-1?", "Omicron", "BA.1", "BA.2", "BA.1.1", "BA.2.12.1", "BA.3", "BA.4", "BA.5", "BA.4/5","BA.2.75", "Mean", "n", "y")
...
clean_up_data <- function(data, wpd = F) {
  
  data$standardised_cell <- gsub("HEK", "", data$standardised_cell)
  
  data <- data[!(is.na(data$`Titre drop`)),]
  
  data <- data %>%
    filter(!(Study %in% c("Behrens", "Thomson"))) %>%
    filter(standardise_encounters != "1.0") %>%
    filter(!is.na(standardise_encounters))
  
  # combine all WTs, all D614Gs
  data$`Comparator antigen`[data$`Comparator antigen` %in% c("WT","WA1", "Wu-1", "Wu-1?", "Victoria")] <- "WT"
  data$`Comparator antigen`[data$`Comparator antigen` %in% c("D614G", "B.1")] <- "D614G"
  
  # combine ba.4/ba.5
  data$OmicronVariant[data$OmicronVariant %in% c("BA.4", "BA.5")] <- "BA.4/5"
  
  data$vacc_type <- gsub("BA.4 conv", "BA.4/5 conv", data$vacc_type)
  data$vacc_type <- gsub("BA.5 conv", "BA.4/5 conv", data$vacc_type)
  
  data$vaccine_manufacturer <- gsub("BA.4 conv", "BA.4/5 conv", data$vaccine_manufacturer)
  data$vaccine_manufacturer <- gsub("BA.5 conv", "BA.4/5 conv", data$vaccine_manufacturer)
  
  data$standardise_encounters <- gsub("BA.4 conv", "BA.4/5 conv", data$standardise_encounters)
  data$standardise_encounters <- gsub("BA.5 conv", "BA.4/5 conv", data$standardise_encounters)
...
factorise <- function(forest_data){
  ...
  forest_data$standardised_cell <- factor(forest_data$standardised_cell, levels = rev(c("Vero","Vero TMPRSS2", "Vero ACE2/TMPRSS2", "Vero-E6", "Vero-E6 TMPRSS2","Vero-E6 ACE2/TMPRSS2","Vero76",
                                                                                        "293T-ACE2/TMPRSS2", "293T-ACE2", "293T/17-ACE2", "293T",
                                                                                        "H1299-ACE2","H1299-E3", "HT1080/ACE2", "S-Fuse", "Caco-2", 
                                                                                        "Huh 7","HOS-ACE2/TMPRSS2", "MDA-MB-231-hACE2","HAT-24", "N/A","Unknown")))

  forest_data$standardised_assay <- factor(forest_data$standardised_assay, levels = rev(c("Live-virus", "Pseudovirus", "Surrogate Virus", "Spike", "N/A")))
 
  forest_data$standardised_pseudo <- factor(forest_data$standardised_pseudo, levels = rev(c("Live-virus", "Lentiviral","Lenti-CMV","VSV","HIV-1","Pseudovirus", "Spike", "N/A")))
  
  forest_data$standardise_encounters <- as.character(forest_data$standardise_encounters)
  
  forest_data$standardise_encounters <- factor(forest_data$standardise_encounters, levels = c(rev(c("WT conv", "Alpha conv", "Beta conv", "Gamma conv", "Delta conv", "BA.1 conv", "BA.2 conv", "BA.4/5 conv")),c("conv","vacc+inf", "inf+vacc", "vacc+ba.1", "vacc+ba.2","vacc+ba.4/5","4", "3", "2", "1")))
  
  forest_data$vacc_type <- factor(forest_data$vacc_type, levels = rev(c("mRNA", "az+mRNA", "az", "j&j","heterologous","other+mRNA", "other", "inf+vacc", "vacc+inf","vacc+ba.1", "vacc+ba.2","vacc+ba.4/5", "conv", "WT conv", "Alpha conv", "Beta conv", "Gamma conv", "Delta conv", "BA.1 conv", "BA.2 conv", "BA.4/5 conv")))
  
  forest_data$vaccine_manufacturer <- factor(forest_data$vaccine_manufacturer, levels = rev(c("pfizer", "moderna", "mRNA", "az", "j&j", "heterologous", "other", "inf+vacc", "vacc+inf","vacc+ba.1", "vacc+ba.2", "vacc+ba.4/5","conv",  "WT conv", "Alpha conv", "Beta conv", "Gamma conv", "Delta conv", "BA.1 conv", "BA.2 conv", "BA.4/5 conv")))

  forest_data$`Comparator antigen` <- factor(forest_data$`Comparator antigen`, levels = c("WT","Wu-1", "WA1",
                                                                                          "D614G","B.1", "Wu-1?",
                                                                                          "Alpha", "Beta", "Gamma", "Delta", "BA.1"))
...
melt_variant_comparator <- function(data, variant) {
  ...
  no_na_rows <- !(is.na(data[,paste0("Titers", variant)]) & is.na(data[,paste0("Titre drop ", variant)]))
   
 
  #1st get the numerical titre drops, if no variant titers are given
  variant_drops <- data[no_na_rows,]
  
  if(nrow(variant_drops) == 0) {
    return(data)
  }
  
  variant_drops <- as.data.frame(variant_drops)
  variant_drops$`Comparator antigen` <- variant
 # variant_drops$`TitersHAg` <-  as.numeric(variant_drops[,paste0("Titers", variant)])
...
character_rowlabel_standard_time <- function(forest_data) {
  ...
  forest_data$time <- unlist(lapply(1:nrow(forest_data), function(x) {
    if(is.na(forest_data$time[x]) | is.na(forest_data$begin[x])) {
      return("")
...
pretty_plot_names <- function(data){
  ...
  pretty_names_encounters <- c("1" =  "1x Vax", "2" =  "2x Vax", "3" = "3x Vax", "4" = "4x Vax", "inf+vacc"= "Inf + Vax", "vacc+inf" = "Vax + Inf", 
                               "vacc+ba.1" = "Vax + BA.1", "vacc+ba.2" = "Vax + BA.2", "vacc+ba.4/5" = "Vax + BA.4/5",
                               "WT conv" =  "WT conv" ,"Alpha conv" =  "Alpha conv", 
                               "Beta conv" = "Beta conv","Gamma conv"= "Gamma conv", "Delta conv"= "Delta conv", "conv" = "Conv",
                               "BA.1 conv" = "BA.1 conv", "BA.2 conv" = "BA.2 conv", "BA.4/5 conv" = "BA.4/5 conv")
  
  pretty_names_vax_manuf <- c(rev(c("inf+vacc"= "Inf + Vax", "vacc+inf" = "Vax + Inf",
                                    "vacc+ba.1" = "Vax + BA.1", "vacc+ba.2" = "Vax + BA.2", "vacc+ba.4/5" = "Vax + BA.4/5",
                                    "WT conv" =  "WT conv" ,"Alpha conv" =  "Alpha conv", 
                                    "Beta conv" = "Beta conv","Gamma conv"= "Gamma conv", "Delta conv"= "Delta conv", "conv" = "Conv",
                                    "BA.1 conv" = "BA.1 conv", "BA.2 conv" = "BA.2 conv", "BA.4/5 conv" = "BA.4/5 conv")), 
  ...
  pretty_names_vax_type <- c(rev(c("inf+vacc"= "Inf + Vax", "vacc+inf" = "Vax + Inf",
                                   "vacc+ba.1" = "Vax + BA.1", "vacc+ba.2" = "Vax + BA.2", "vacc+ba.4/5" = "Vax + BA.4/5",
                                   "WT conv" =  "WT conv" ,"Alpha conv" =  "Alpha conv", 
                                    "Beta conv" = "Beta conv","Gamma conv"= "Gamma conv", "Delta conv"= "Delta conv", "conv" = "Conv",
                                   "BA.1 conv" = "BA.1 conv", "BA.2 conv" = "BA.2 conv", "BA.4/5 conv" = "BA.4/5 conv")), 
                              c("heterologous" = "Het", "other" = "Other", "j&j" = "J&J", "az" = "AZ","az+mRNA"="AZ + mRNA","other+mRNA" = "other + mRNA", "mRNA" = "mRNA"))
  
  pretty_names_assay <- c("Live-virus" = "LV", "Pseudovirus" = "PV", "Surrogate Virus" = "SV", "Spike" = "Spike", "N/A" = "N/A")
...
vertical_labelling_plot <- function(base_plot, data, hline_by, hline_positions, axis_text_size, mean_data = NULL) {
  ...
  label_pos <- unlist(lapply(1:(length(breaks)-1), function(x) {
    ((breaks[x] + breaks[x+1])/2)
  }))
  ...
  test_plot + 
    ...
    annotate(geom = "text", label = paste0("\n\n",mean_labels),
             x = rep(2.4, length(mean_labels)), 
             y = label_pos, 
             size = (text_size-1.8)/figure_factor_increase,
             hjust = 1) +
...
calc_mean_per_grouping <- function(data, hline_by, meantiter_package = FALSE) {
  
  temp_data <- data
  temp_data$`Comparator antigen`[(temp_data$`Comparator antigen` %in% c("WT", "D614G"))] <- "WT"
  ...
  #          ) 
  # 
  if(meantiter_package){
    
    means <- temp_data %>%
      group_by(across(all_of(c("Comparator antigen", hline_by, "OmicronVariant")))) %>%
      summarise(fc_lower = Rmisc::CI(log_fold_change[complete.cases(log_fold_change)])["lower"],
                fc_upper = Rmisc::CI(log_fold_change[complete.cases(log_fold_change)])["upper"],
                log_fold_change= mean(log_fold_change, na.rm = TRUE),
                ...
                # Log2HAg = mean(Log2HAg,na.rm = T),
                # Log2Omi = mean(Log2Omi, na.rm = T),
                arrow_length = mean(arrow_length, na.rm = T),
                rowlabel = paste0("Mean ", OmicronVariant),
                Webplotdigitizer = "n",
                lower_hag = mean_titers(TitersHAg, method= "truncated_normal", dilution_stepsize = 0)$mean_lower,
                higher_hag = mean_titers(TitersHAg, method= "truncated_normal", dilution_stepsize = 0)$mean_upper,
                lower_omi  = mean_titers(TitersOmicron, method= "truncated_normal", dilution_stepsize = 0)$mean_lower,
                higher_omi  = mean_titers(TitersOmicron, method= "truncated_normal", dilution_stepsize = 0)$mean_upper
      ) %>% ungroup() %>% unique()
    ...
    means <- temp_data %>%
      ...
      summarise(fc_lower = Rmisc::CI(log_fold_change[complete.cases(log_fold_change)])["lower"],
                ...
                rowlabel = paste0("Mean ", OmicronVariant),
                Webplotdigitizer = "n",
                lower_hag = Rmisc::CI(na.omit(Log2HAg))[["lower"]],
                higher_hag = Rmisc::CI(na.omit(Log2HAg))[["upper"]],
                lower_omi  = Rmisc::CI(na.omit(Log2Omi))[["lower"]],
                higher_omi  = Rmisc::CI(na.omit(Log2Omi))[["upper"]],
                Log2HAg = Rmisc::CI(na.omit(Log2HAg))[["mean"]],
                Log2Omi = Rmisc::CI(na.omit(Log2Omi))[["mean"]]
      ) %>% ungroup() %>% unique()
  ...
  means$mean_value <- unlist(lapply(1:nrow(means), function(x) {
    if(!is.na(means$arrow_length[x])) {
      if(means$arrow_length[x] > 2/3) {
        paste0("Mean = <<", round(2^means$Log2Omi[x]*10, 0), ", >>", round(2^means$log_fold_change[x], 1), "x")
      } else if(means$arrow_length[x] > 1/3) {
        paste0("Mean = <", round(2^means$Log2Omi[x]*10, 0), ", >", round(2^means$log_fold_change[x], 1), "x")
      } else {
        paste0("Mean = ", round(2^means$Log2Omi[x]*10, 0), ", ", round(2^means$log_fold_change[x], 1), "x")
      }
  ...
  means$Log2HAg[means$Log2HAg < 0] <- -1
  means$Log2Omi[means$Log2Omi < 0] <- -1
...
# if(comp_antigen %in% c("Alpha", "Beta", "Gamma", "Delta")) {
#    return(plot(c(1:10), c(1:10)))
#  }
  if("B.1" %in% comp_antigen | "D614G" %in% comp_antigen) {
    comp_antigen <- c(comp_antigen, "WT")
  }
  
  mean_data <- calc_mean_per_grouping(forest_data, hline_by)
  mean_data <- mean_data[grepl("Mean", mean_data$rowlabel),]
 ...
 # print(hline_positions)
  ...
  forest_data %>% ggplot(
    aes(
      x = log_fold_change,
      y = Row_long,
      color = OmicronVariant, #`Comparator antigen`,
      fill = SAVE_lab,
      shape = shape_col
    )
  ) + scale_y_discrete(
    ...
    scale_x_reverse(
      labels = function(x) ifelse(x <= 0, 2^abs(x), paste0("-", 2^x)),
  ...
  label_plot <- vertical_labelling_plot(base_plot = base_plot, data = forest_data, hline_by = hline_by, hline_positions = hline_positions, axis_text_size = axis_text_size, mean_data = mean_data)
...
do_ve_plot <- function(data, hline_by = "", row_label, show_mean, axis_text_size, order_by_omicron = FALSE) {
  ...
  mean_data <- calc_mean_per_grouping(data, hline_by)
  mean_data <- mean_data[grepl("Mean", mean_data$rowlabel),]
  ...
  if(!row_label) {
    if(show_mean) {
      mean_data <- data[grepl("Mean", data$rowlabel),]
    }
  ...
  base_plot +
    ...
    geom_hline(yintercept = hline_positions, size =hline_size, color = "white") + 
    geom_point(aes(color = ifelse(SAVE_lab == "y", "lightblue", OmicronVariant), fill = OmicronVariant), size = point_size)+#, shape = 21) + 
  ...
  label_plot <- vertical_labelling_plot(base_plot = base_plot, data = data, hline_by = hline_by, hline_positions = hline_positions, axis_text_size = axis_text_size, mean_data = mean_data)
...
split_up_forest_plot_drops <- function(data, hline_by = "", row_label = TRUE, show_means = FALSE, axis_text_size = axis_text_size_p, single_variant = FALSE) {
  ...
  if(length(single_variant)==0) {
   ...
   # ba1 <- data %>% filter(`Comparator antigen` %in% c("BA.1")) %>% do_forest_titer_drop(hline_by, row_label, show_means_var, axis_text_size)
    ...
    return(list("WT" = wt, "Alpha" = alpha, "Beta" = beta, "Delta" = delta, "BA.1" = ba1))
  } else {
      if(single_variant %in%c("WT","Wu-1", "WA1","D614G","B.1", "Wu-1?")) {
        wt <- data %>% filter(`Comparator antigen` %in% c("WT","Wu-1", "WA1","D614G","B.1", "Wu-1?")) %>% do_forest_titer_drop(hline_by, row_label, show_means, axis_text_size)
        
        return(list("single_variant" = wt))
      } else {
        plot <- data %>% filter(`Comparator antigen` ==single_variant) %>% do_forest_titer_drop(hline_by, row_label, show_means_var, axis_text_size)
        
        return(list("single_variant" = plot))
      }
  }
  
} 

split_up_forest_plot_titer <- function(data, hline_by = "", row_label = TRUE, show_means = FALSE, axis_text_size = axis_text_size_p, order_by_omi = FALSE, single_variant = FALSE) {
  show_means_var <- show_means
  if(length(hline_by) > 1) {
    show_means_var <- FALSE
  }
  ...
  if(length(single_variant)==0) {
    ...
    if("BA.1" %in% data$`Comparator antigen`) {
      ba1 <- data %>% filter(`Comparator antigen` %in% c("Delta")) %>% do_ve_plot(hline_by, row_label, show_means_var,axis_text_size, order_by_omicron = order_by_omi)
    }
    # wt <- data %>% filter(`Comparator antigen` %in% c("WT","Wu-1", "WA1","D614G","B.1", "Wu-1?")) %>% do_ve_plot(hline_by, row_label, show_means, axis_text_size, order_by_omicron = order_by_omi)
    # beta <- data %>% filter(`Comparator antigen` %in% c("Beta")) %>% do_ve_plot(hline_by, row_label, show_means_var, axis_text_size, order_by_omicron = order_by_omi)
    # alpha <- data %>% filter(`Comparator antigen` %in% c("Alpha")) %>% do_ve_plot(hline_by, row_label, show_means_var,axis_text_size, order_by_omicron = order_by_omi)
    ...
    if(single_variant %in%c("WT","Wu-1", "WA1","D614G","B.1", "Wu-1?")) {
      ...
      plot <-  data %>% filter(`Comparator antigen`== single_variant) %>% do_ve_plot(hline_by, row_label, show_means_var, axis_text_size, order_by_omicron = order_by_omi)
...
save_in_scaled_format <- function(data, which_plot, width = 8, height = 12, 
                                  to_save= "plots", hline_by = "", single_plots = TRUE, 
                                  axis_text_size=axis_text_size_p, 
                                  order_by_omicron = FALSE, show_mean = TRUE, single_variant = NULL,
                                  height_wt =12,
                                  height_alpha = 4,
                                  height_beta = 4,
                                  height_delta = 6,
  ...
  if(which_plot == "titer_drop") {
    plots <- split_up_forest_plot_drops(data, hline_by, row_label = TRUE, show_means = show_mean, axis_text_size, single_variant = single_variant)
    if(hline_by[1] == "standardise_encounters") {
      plots_no_lab <- split_up_forest_plot_drops(data, hline_by, row_label = FALSE, show_means = show_mean, axis_text_size, single_variant = single_variant)
    }
    
    
    
    to_save <- file.path(path_to_save,paste0(to_save, "_fold_drops"))
    ...
    plots <- split_up_forest_plot_titer(data, hline_by, row_label = TRUE, show_means = show_mean, axis_text_size, order_by_omi = order_by_omicron, single_variant = single_variant)
    if(hline_by[1] == "standardise_encounters") {
      plots_no_lab <- split_up_forest_plot_titer(data, hline_by, row_label = FALSE, show_means = show_mean, axis_text_size, order_by_omi = order_by_omicron, single_variant = single_variant)
    }
    
    to_save <- file.path(path_to_save,paste0(to_save, "_titers"))
  ...
  if(length(single_variant)==0) {
    ...
    ggsave(paste0(to_save, "_WT.",fileext), plots$WT, width = width, height = height_wt, dpi = 300)
    ggsave(paste0(to_save, "_Alpha.",fileext), plots$Alpha, width = width, height = height_alpha, dpi = 300)
    ggsave(paste0(to_save, "_Beta.",fileext), plots$Beta, width = width, height = height_beta, dpi = 300)
    ggsave(paste0(to_save, "_Delta.",fileext), plots$Delta, width = width, height = height_delta, dpi = 300)
    ggsave(paste0(to_save, "_BA1.",fileext), plots$BA1, width = width, height = height_ba1, dpi = 300)
    
    if(hline_by[1] == "standardise_encounters") {
      ggsave(paste0(to_save, "_no_lab_WT.",fileext), plots_no_lab$WT, width = width, height = height_wt, dpi = 300)
    }
  } else {
    ggsave(paste0(to_save, "_", single_variant,".",fileext), plots$single_variant, width = width, height = height_wt, dpi = 300)
  }
}
...
format_huxtable_cols_standard <- function(ht, single_grouping = TRUE, outer_group = "", target_ag = "Omicron", only_omicron = FALSE, comp_ags = NULL) {
  base <- 1
  ...
  if(only_omicron){
    ...
    ht %>%
      merge_cells(.,1, (base+1):(base+1)) %>%
      merge_cells(.,1, (base+2):(base+2+length(comp_ags)-1)) %>%
      merge_cells(.,1:2,base) -> ht
    
    ht[1,base] <- "Serum Group"
    ht[1,base+1] <- "GMT"
  #  ht[1,base+2] <- paste0("Mean ", target_ag, " fold drop from")
    ht[1,base+2] <- "Mean Omicron fold drop from"
    ht[2,(base+1)] <- c(target_ag)
    ht[2,(base+2):(base+2+length(comp_ags)-1)] <- comp_ags
    ...
    ht %>% set_align(1, everywhere, "center") %>%
      ...
      set_right_border(everywhere, base+1, 1) %>% 
      set_right_border_color(everywhere, base, "grey")  %>%
      set_right_border_color(everywhere, base+1, "grey")   -> ht
    ...
    ht[1,base] <- "Serum Group"
    ht[1,base+1] <- "GMT"
    #  ht[1,base+7] <- paste0("Mean ", target_ag, " fold drop from")
    ht[1,base+7] <- "Mean Omicron fold drop from"
    ht[1,base+12] <- "Mean fold drop to WT"
    ht[2,(base+1):(base+6)] <- c("WT", "Alpha", "Beta", "Gamma", "Delta", target_ag)
